...
def test_bdg_initial_direction():
    ...
    with warnings.catch_warnings():
        ...
        boot_dg = BootDirectionGetter.from_data(
            voxel, dti_model, 30, sphere=sphere, sh_order=6
        )
    ...
    with warnings.catch_warnings():
        ...
        csd_model = ConstrainedSphericalDeconvModel(gtab, response=response, sh_order=4)
        boot_dg = BootDirectionGetter.from_data(
            voxel,
            csd_model,
            30,
...
def test_bdg_get_direction():
    ...
    boot_dg = BootDirectionGetter(pmf_gen, angle / 2.0, sphere=sphere)
    ...
    boot_dg = BootDirectionGetter(pmf_gen, angle / 2.0, sphere=sphere, max_attempts=3)
    npt.assert_equal(boot_dg.get_direction(point, prev_direction), 1)
    npt.assert_equal(direction2, prev_direction)

    # test case in which a valid direction is found
    boot_dg = BootDirectionGetter(pmf_gen, angle * 2.0, sphere=sphere, max_attempts=1)
    ...
    npt.assert_raises(
        ValueError,
        lambda: BootDirectionGetter(
            pmf_gen, angle * 2.0, sphere=sphere, max_attempts=0
        ),
    )
...
def test_bdg_residual():
    ...
    with warnings.catch_warnings():
        ...
        boot_pmf_gen = BootPmfGen(
            data, model=csd_model, sphere=hsph_updated, sh_order=6
        )
    ...
    with warnings.catch_warnings():
        ...
        boot_pmf_gen2 = BootPmfGen(
            data, model=csd_model, sphere=hsph_updated, sh_order=4
        )
