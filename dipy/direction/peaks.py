...
def peak_directions(odf, sphere, relative_peak_threshold=0.5, min_separation_angle=25):
    ...
    odf_min = np.min(odf)
    odf_min = odf_min if (odf_min >= 0.0) else 0.0
...
def _peaks_from_model_parallel(
    ...
    indices = list(
        zip(np.arange(0, n, chunk_size), np.arange(0, n, chunk_size) + chunk_size)
    )
