...
def test_enhancement_kernel():
    ...
    kernelvalues = [
        0.10701063104295713,
        0.0030052117308328923,
        0.003125410084676201,
        0.0031765819772012613,
        0.003127254657020615,
        0.0001295130396491743,
        6.882352014430076e-14,
        1.3821277371353332e-13,
        1.3951939946082493e-13,
        1.381612071786285e-13,
        5.0861109163441125e-17,
        1.0722120295517027e-10,
        2.425145934791457e-6,
        3.557919265806602e-6,
        3.6669510385105265e-6,
        5.97473789679846e-11,
        6.155412262223178e-11,
    ]
...
def test_spike():
    ...
    num_orientations = 5
    k = EnhancementKernel(
        D33, D44, t, orientations=num_orientations, force_recompute=True
    )
    ...
    for i in range(0, numorientations):
        totalsum += np.sum(
            np.array(k.get_lookup_table())[i, 0, :, :, :]
            - np.array(csd_enh)[:, :, :, i]
        )
