...
def test_ascm_static():
    ...
    S0n1 = non_local_means(S0, sigma=0, rician=False, patch_radius=1, block_radius=1)
    S0n2 = non_local_means(S0, sigma=0, rician=False, patch_radius=2, block_radius=1)
...
def test_ascm_random_noise():
    ...
    S0n1 = non_local_means(S0, sigma=1, rician=False, patch_radius=1, block_radius=1)
    S0n2 = non_local_means(S0, sigma=1, rician=False, patch_radius=2, block_radius=1)
...
def test_ascm_rmse_with_nlmeans():
    ...
    S0n1 = non_local_means(
        S0_noise, sigma=400, rician=False, patch_radius=1, block_radius=1
    )
    print("Smaller patch RMSE", np.sum(np.abs(S0 - S0n1)) / np.sum(S0))
    S0n2 = non_local_means(
        S0_noise, sigma=400, rician=False, patch_radius=2, block_radius=2
    )
...
def test_ascm_accuracy():
    ...
    S0n = np.array(adaptive_soft_matching(test_data, den_small, den_large, sigma[0]))
