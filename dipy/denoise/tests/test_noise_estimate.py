...
def test_piesno():
    ...
    sigma, mask = piesno(
        rician_noise, N=1, alpha=0.01, l=1, eps=1e-10, return_mask=True
    )
    ...
    sigma = _piesno_3D(
        1000 * np.ones_like(rician_noise),
        N=1,
        alpha=0.01,
        l=1,
        eps=1e-10,
...
def test_pca_noise_estimate():
    ...
    assert_(
        np.mean(pca_noise_estimate(data, gtab, correct_bias=True))
