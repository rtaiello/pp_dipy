    ...
    with nogil:
        ...
        for i in range(nr):
            for j in range(nc):
                ...
                den = mean_delta_sq/sigma_sq_x + nrm2
                if den <1e-9:
                    out[i, j, 0] = 0
                    out[i, j, 1] = 0
                else:
...
@cython.wraparound(False)
@cython.cdivision(True)
def pp_compute_ssd_demons_step_3d(floating[:,:,:] image,
                               floating[:,:,:,:] gradient,
                               double sigma_sq_x,
                               double mean_delta_sq,
                               floating[:,:,:,:] out):
    cdef:
        cnp.npy_intp ns = image.shape[0]
        cnp.npy_intp nr = image.shape[1]
        cnp.npy_intp nc = image.shape[2]
        cnp.npy_intp i, j, k
        double delta, delta_2, nrm2, energy, den
            ...
            for i in range(nr):
                for j in range(nc):
                    ...
                    if den < 1e-9:
                        ...
                        out[k, i, j, 0] = (delta *
                                           gradient[k, i, j, 0] / den)
                        out[k, i, j, 1] = (delta *
                                           gradient[k, i, j, 1] / den)
                        out[k, i, j, 2] = (delta *
                                           gradient[k, i, j, 2] / den)
    return np.asarray(out)