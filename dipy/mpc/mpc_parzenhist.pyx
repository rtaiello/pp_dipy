...
class ParzenJointHistogramMPC:
    ...
    def setup(self, static, moving, smask=None, mmask=None):
        ...
        self.mpc_joint_grad = None
        self.metric_grad = None
        self.metric_val = 0
        self.joint = np.zeros(shape=(self.nbins, self.nbins))
        self.mpc_joint = np.zeros(shape=(self.nbins, self.nbins))
    ...
    def update_pdfs_dense(self, static, moving, smask=None, mmask=None):
        ...
        if dim == 2:
            ...
            _compute_pdfs_dense_3d(static, moving, smask, mmask, self.smin,
                                   self.sdelta, self.mmin, self.mdelta,
                                   self.nbins, self.padding, self.joint, self.mpc_joint,
                                   self.smarginal, self.mmarginal)
    ...
    def update_pdfs_sparse(self, sval, mval):
        ...
        start = time.time()
        num_bits_pdf = _compute_pdfs_sparse(sval, mval, self.smin, self.sdelta,
                                      self.mmin, self.mdelta, self.nbins,
                                      self.padding, self.joint,self.mpc_joint,
                                      self.smarginal, self.mmarginal)
        end = time.time()
        self.time_pdf += end - start
        self.num_bits_pdf+=num_bits_pdf
    ...
    def update_gradient_dense(self, theta, transform, static, moving,
        ...
        if (self.joint_grad is None) or (self.joint_grad.shape[2] != n):
            ...
            self.mpc_joint_grad = np.zeros((nbins, nbins, n))
        if dim == 2:
            ...
            if mgradient.dtype == np.float64:
                _joint_pdf_gradient_dense_3d[cython.double](theta, transform,
                    static, moving, grid2world, mgradient, smask, mmask,
                    self.smin, self.sdelta, self.mmin, self.mdelta,
                    self.nbins, self.padding, self.joint_grad, self.mpc_joint_grad)
            elif mgradient.dtype == np.float32:
    ...
    def update_gradient_sparse(self, theta, transform, sval, mval,
        ...
        if (self.joint_grad is None) or (self.joint_grad.shape[2] != n):
            ...
            self.mpc_joint_grad = np.zeros(shape=(nbins, nbins, n))
        ...
        if dim == 2:
            ...
            if mgradient.dtype == np.float64:
                start = time.time()
                num_bits_grad = _joint_pdf_gradient_sparse_3d[cython.double](theta, transform,
                    sval, mval, sample_points, mgradient, self.smin,
                    self.sdelta, self.mmin, self.mdelta, self.nbins,
                    self.padding, self.joint_grad, self.mpc_joint_grad)
                end = time.time()
            elif mgradient.dtype == np.float32:
                start = time.time()
                num_bits_grad = _joint_pdf_gradient_sparse_3d[cython.float](theta, transform,
                    sval, mval, sample_points, mgradient, self.smin,
                    self.sdelta, self.mmin, self.mdelta, self.nbins,
                    self.padding, self.joint_grad, self.mpc_joint_grad)
                end = time.time()
...
cdef _compute_pdfs_dense_3d(double[:, :, :] static, double[:, :, :] moving,
                            ...
                            double mmin, double mdelta,
                            int nbins, int padding, double[:, :] joint,
                            double[:,:] mpc_joint, double[:] smarginal, double[:] mmarginal):
    ...
    cdef:
        ...
        double rn, cn
        double val, spline_arg, total_sum
        double[:, :, :, :] cubic_spline_3d = np.zeros((nslices, nrows, ncols, nbins), dtype=np.float64)
        cnp.npy_uint8[:, :, :, :] zero_spline_3d = np.zeros((nslices, nrows, ncols, nbins), dtype=np.uint8)
        double[:,:] cubic_spline_hist = np.zeros((nslices*nrows*ncols, nbins), dtype=np.float64)
        cnp.npy_uint8[:,:] zero_spline_hist = np.zeros((nbins, nslices*nrows*ncols), dtype=np.uint8)
    joint[...] = 0
    mpc_joint[...] = 0
    total_sum = 0
    num_bits = 0
        ...
        for k in range(nslices):
            for i in range(nrows):
                for j in range(ncols):
                    ...
                    zero_spline_3d[k, i, j, r] = 1
                    smarginal[r] += 1
                    for offset in range(-2, 3):
                        val = _cubic_spline(spline_arg)
                        cubic_spline_3d[k, i, j, c + offset] = val
                        joint[r, c + offset] += val
                        total_sum += val
                        spline_arg += 1.0

    cubic_spline_hist = np.reshape(cubic_spline_3d,(valid_points, nbins))
    zero_spline_hist = np.transpose(np.reshape(zero_spline_3d, (valid_points, nbins)))
    with nogil:
        triple = generate_mul_triple()
        for i in range(zero_spline_hist.shape[0]):
            for j in range(cubic_spline_hist.shape[1]):
                share_value = share(encode(0.0))
                num_bits += SHARE_COMM
                for k in range(valid_points):
                    share_zero_spline = share(encode(<double> zero_spline_hist[i,k]))
                    num_bits += SHARE_COMM
                    share_cubic_spline = share(encode(cubic_spline_hist[k,j]))
                    num_bits += SHARE_COMM
                    share_value = add(share_value, truncate(mul(share_zero_spline, share_cubic_spline, triple)))
                    num_bits += MUL_COMM
                mpc_joint[i,j] = decode(reconstruct(share_value))
                num_bits += RECON_COMM
        if total_sum > 0:
            for i in range(nbins):
                for j in range(nbins):
                    joint[i, j] /= total_sum
                    mpc_joint[i, j] /= total_sum
...
cdef _compute_pdfs_sparse(double[:] sval, double[:] mval, double smin,
                          ...
                          int nbins, int padding, double[:, :] joint, double[:, :] mpc_joint,
                          double[:] smarginal, double[:] mmarginal):
    ...
    cdef:
        ...
        double rn, cn
        double val, spline_arg, total_sum
        Share[:,:] share_joint = np.zeros((nbins, nbins), dtype=ShareData)
    joint[...] = 0
    mpc_joint[...] = 0
    total_sum = 0
    num_bits = 0
    ...
    with nogil:
        ...
        for i in range(n):
            ...
            for offset in range(-2, 3):
                val = _cubic_spline(spline_arg)
                # joint[r, c + offset] += val
                share_cubic_spline = share(encode(val))
                num_bits += SHARE_COMM
                share_zero_spline = share(encode(1.0))
                num_bits += SHARE_COMM
                share_joint[r, c + offset] = add(share_joint[r, c + offset], truncate(mul(share_zero_spline, share_cubic_spline, triple)))
                num_bits += MUL_COMM
                joint[r, c + offset] = decode(reconstruct(share_joint[r, c + offset]))
                num_bits += RECON_COMM
                total_sum += val
                spline_arg += 1.0
...
cdef _joint_pdf_gradient_dense_3d(double[:] theta, Transform transform,
                                  ...
                                  double[:, :, :] grad_pdf, double[:, :, :] mpc_grad_pdf):
    r""" Gradient of the joint PDF w.r.t. transform parameters theta
    ...
    cdef:
        ...
        cnp.npy_uint8[:, :, :, :] zero_spline_3d = np.zeros((nslices, nrows, ncols, nbins), dtype=np.uint8)
        double[:, :, :, :, :] derivative_cubic_spline_3d = np.zeros((nslices, nrows, ncols, nbins, n), dtype=np.float64) 
        cnp.npy_uint8[:,:] zero_spline_hist = np.zeros((nslices*nrows*ncols, nbins), dtype=np.uint8)
        double[:, :, :] derivative_cubic_spline_hist = np.zeros((nslices*nrows*ncols, nbins, n), dtype=np.float64)
                ...
                for j in range(ncols):
                    ...
                    for offset in range(-2, 3):
                        ...
                        for l in range(n):
                            ...
                            derivative_cubic_spline_3d[k, i, j, c + offset, l] = -(val * prod[l])
                        spline_arg += 1.0
    zero_spline_hist = np.transpose(np.reshape(zero_spline_3d, (valid_points, nbins)))
    derivative_cubic_spline_hist = np.transpose(np.reshape(derivative_cubic_spline_3d, (valid_points, nbins, n)))
    with nogil:
        triple = generate_mul_triple()
        for i in range(zero_spline_hist.shape[0]):
            for j in range(derivative_cubic_spline_hist.shape[1]):
                for k in range(derivative_cubic_spline_hist.shape[2]):
                    share_value = share(encode(0.0))
                    for l in range(valid_points):
                        share_zero_spline = share(encode(<double> zero_spline_hist[i,l]))
                        share_derivative_cubic_spline = share(encode(derivative_cubic_spline_hist[l,j,k]))
                        share_value = add(share_value, truncate(mul(share_zero_spline, share_derivative_cubic_spline, triple)))
                    mpc_grad_pdf[i,j,k] = decode(reconstruct(share_value))
        norm_factor = valid_points * mdelta
        if norm_factor > 0:
            for i in range(nbins):
                for j in range(nbins):
                    for k in range(n):
                        grad_pdf[i, j, k] /= norm_factor
                        mpc_grad_pdf[i, j, k] /= norm_factor
    grad_pdf = mpc_grad_pdf
...
cdef _joint_pdf_gradient_sparse_3d(double[:] theta, Transform transform,
    ...
    cdef:
        ...
        double[:, :] J = np.empty(shape=(3, n), dtype=np.float64)
        double[:] prod = np.empty(shape=(n,), dtype=np.float64)
        Share[:, :, :] share_grad_pdf = np.zeros((nbins, nbins, n), dtype=ShareData)
    grad_pdf[...] = 0
    mpc_grad_pdf[...] = 0
    num_bits = 0
    with nogil:
        ...
        for i in range(m):
            ...
            for offset in range(-2, 3):
                ...
                for j in range(n):
                    # grad_pdf[r, c + offset, j] -= val * prod[j]
                    share_derivative_cubic_spline = share(encode(val * prod[j]))
                    num_bits += SHARE_COMM
                    share_zero_spline = share(encode(-1.0))
                    num_bits += SHARE_COMM
                    share_grad_pdf[r, c + offset, j] = add(share_grad_pdf[r, c + offset, j], truncate(mul(share_zero_spline, share_derivative_cubic_spline, triple)))
                    num_bits += MUL_COMM
                    grad_pdf[r, c + offset, j] = decode(reconstruct(share_grad_pdf[r, c + offset, j]))
                    num_bits += RECON_COMM
                spline_arg += 1.0
        norm_factor = valid_points * mdelta
        if norm_factor > 0:
