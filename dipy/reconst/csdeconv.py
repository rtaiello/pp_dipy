...
class ConstrainedSphericalDeconvModel(SphHarmModel):
    def __init__(
        ...
        self.B_dwi = real_sh_descoteaux_from_index(m, n, theta[:, None], phi[:, None])
        ...
        self.B_reg = real_sh_descoteaux_from_index(m, n, theta[:, None], phi[:, None])
...
class ConstrainedSDTModel(SphHarmModel):
    def __init__(self, gtab, ratio, reg_sphere=None, sh_order=8, lambda_=1.0, tau=0.1):
...
def forward_sdt_deconv_mat(ratio, n, r2_term=False):
    ...
    for l in np.arange(0, n_degrees * 2, 2):
        if r2_term:
            ...
            sharp = quad(
                lambda z: lpn(l, z)[0][-1] * np.sqrt(1 / (1 - (1 - ratio) * z * z)),
                -1.0,
                1.0,
            )
...
def mask_for_response_ssst(gtab, data, roi_center=None, roi_radii=10, fa_thr=0.7):
    ...
    if np.sum(mask) == 0:
        ...
        Try a larger roi or a lower threshold.""".format(
            str(fa_thr)
        )
        warnings.warn(msg, UserWarning)
