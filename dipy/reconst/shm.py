...
def real_sh_tournier(sh_order, theta, phi, full_basis=False, legacy=True):
    """Compute real spherical harmonics as initially defined in Tournier
    2007 [1]_ then updated in MRtrix3 [2]_, where the real harmonic $Y^m_n$
    is defined to be:
...
def real_sh_descoteaux(sh_order, theta, phi, full_basis=False, legacy=True):
    """Compute real spherical harmonics as in Descoteaux et al. 2007 [1]_,
    where the real harmonic $Y^m_n$ is defined to be:
...
class QballBaseModel(SphHarmModel):
    ...
    def __init__(
        self, gtab, sh_order, smooth=0.006, min_signal=1e-5, assume_normed=False
    ):
...
def sf_to_sh(
    sf, sphere, sh_order=4, basis_type=None, full_basis=False, legacy=True, smooth=0.0
):
    ...
    B, m, n = sph_harm_basis(
        sh_order, sphere.theta, sphere.phi, full_basis=full_basis, legacy=legacy
    )
...
def sh_to_sf(sh, sphere, sh_order=4, basis_type=None, full_basis=False, legacy=True):
...
def convert_sh_from_legacy(sh_coeffs, sh_basis, full_basis=False):
    ...
    .. [3] Tournier J-D, Smith R, Raffelt D, Tabbara R, Dhollander T,
           ...
           NeuroImage. 2019 Nov 15;202:116-137.
    """
    sh_order = calculate_max_order(sh_coeffs.shape[-1], full_basis=full_basis)

    m, n = sph_harm_ind_list(sh_order, full_basis=full_basis)

    if sh_basis == "descoteaux07":
        out_sh_coeffs = sh_coeffs * np.where(m < 0, (-1.0) ** m, 1.0)
    elif sh_basis == "tournier07":
        out_sh_coeffs = sh_coeffs * np.where(m == 0, 1.0, 1.0 / np.sqrt(2))
    else:
        raise ValueError("Invalid basis name.")
...
def convert_sh_to_legacy(sh_coeffs, sh_basis, full_basis=False):
    ...
    if sh_basis == "descoteaux07":
        ...
        out_sh_coeffs = sh_coeffs * np.where(m == 0, 1.0, np.sqrt(2))
    else:
        raise ValueError("Invalid basis name.")
