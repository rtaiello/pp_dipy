...
def multi_tissue_basis(gtab, sh_order, iso_comp):
    ...
    B[np.ix_(gtab.b0s_mask, n > 0)] = 0.0
...
class MultiShellDeconvModel(shm.SphHarmModel):
    def __init__(self, gtab, response, reg_sphere=default_sphere, sh_order=8, iso=2):
        r"""
        ...
        delta = _basic_delta(response.iso, response.m, response.n, 0.0, 0.0)
        self.delta = delta
    ...
    def predict(self, params, gtab=None, S0=None):
        ...
        if gtab is None or gtab is self.gtab:
            ...
            multiplier_matrix = _inflate_response(self.response, gtab, n, self.delta)
            X = B * multiplier_matrix
...
def multi_shell_fiber_response(
    sh_order, bvals, wm_rf, gm_rf, csf_rf, sphere=None, tol=20
):
    ...
    if bvals[0] < tol:
        ...
        for i, bvalue in enumerate(bvals[1:]):
            ...
            response[i + 1, 2:] = np.linalg.lstsq(B, wm_response, rcond=rcond_value)[0]
        ...
        for i, bvalue in enumerate(bvals):
            ...
            response[i, 2:] = np.linalg.lstsq(B, wm_response, rcond=rcond_value)[0]
