...
def setup_module():
    ...
    data.mevals = np.array(([0.0017, 0.0003, 0.0003], [0.0017, 0.0003, 0.0003]))
    data.angl = [(0, 0), (60, 0)]
    data.S, data.sticks = multi_tensor(
        data.gtab, data.mevals, S0=100.0, angles=data.angl, fractions=[50, 50], snr=None
    )
    data.sh_order = 6
    data.lambda_lb = 1e-8
    data.lambda_csd = 1.0
...
def test_forecast_csd():
    ...
    with warnings.catch_warnings():
        ...
        fm = ForecastModel(
            data.gtab, dec_alg="CSD", sphere=data.sphere, lambda_csd=data.lambda_csd
        )
    f_fit = fm.fit(data.S)
    with warnings.catch_warnings():
        warnings.filterwarnings(
    ...
    with warnings.catch_warnings():
        ...
        fm = ForecastModel(
            data.gtab, sh_order=data.sh_order, lambda_lb=data.lambda_lb, dec_alg="WLS"
        )
    f_fit = fm.fit(data.S)
    with warnings.catch_warnings():
        warnings.filterwarnings(
...
def test_forecast_odf():
    ...
    with warnings.catch_warnings():
        ...
        fm = ForecastModel(data.gtab, sh_order=4, dec_alg="CSD", sphere=data.sphere)
    f_fit = fm.fit(data.S)
    ...
    with warnings.catch_warnings():
        ...
        fm = ForecastModel(data.gtab, sh_order=6, dec_alg="CSD", sphere=data.sphere)
    f_fit = fm.fit(data.S)
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
    ...
    with warnings.catch_warnings():
        ...
        fm = ForecastModel(data.gtab, sh_order=8, dec_alg="CSD", sphere=data.sphere)
    f_fit = fm.fit(data.S)
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
    ...
    with warnings.catch_warnings():
        ...
        fm = ForecastModel(
            data.gtab, sh_order=10, dec_alg="CSD", sphere=sphere.vertices
        )
    f_fit = fm.fit(data.S)
    with warnings.catch_warnings():
        warnings.filterwarnings(
    ...
    with warnings.catch_warnings():
        ...
        fm = ForecastModel(
            data.gtab, sh_order=12, dec_alg="CSD", sphere=sphere.vertices
        )
    f_fit = fm.fit(data.S)
    with warnings.catch_warnings():
        warnings.filterwarnings(
...
def test_forecast_indices():
    ...
    with warnings.catch_warnings():
        ...
        fm = ForecastModel(
            data.gtab, sh_order=2, lambda_lb=data.lambda_lb, dec_alg="WLS"
        )
    f_fit = fm.fit(data.S)
    ...
    gt_fa = np.sqrt(
        0.5
        * (2 * (data.mevals[0, 0] - data.mevals[0, 1]) ** 2)
    ...
    S, sticks = multi_tensor(
        data.gtab, mevals, S0=100.0, angles=data.angl, fractions=[50, 50], snr=None
    )
...
def test_multivox_forecast():
    ...
    S[0, 0, 0], _ = multi_tensor(
        gtab, mevals, S0=1.0, angles=angl1, fractions=[50, 50], snr=None
    )
    S[1, 0, 0], _ = multi_tensor(
        gtab, mevals, S0=1.0, angles=angl2, fractions=[50, 50], snr=None
    )
    S[2, 0, 0], _ = multi_tensor(
        gtab, mevals, S0=1.0, angles=angl3, fractions=[50, 50], snr=None
    )
