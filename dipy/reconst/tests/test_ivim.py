...
def setup_module():
    global gtab, ivim_fit_single, ivim_model_trr, data_single, params_trr, data_multi, ivim_params_trr, D_star, D, f, S0, gtab_with_multiple_b0, noisy_single, mevals, gtab_no_b0, ivim_fit_multi, ivim_model_VP, f_VP, D_star_VP, D_VP, params_VP
    ...
    noisy_multi[0, 1, 0] = noisy_multi[1, 0, 0] = noisy_multi[1, 1, 0] = noisy_single
...
def test_ivim_errors():
    ...
    """
    ivim_model_trr = IvimModel(
        gtab, bounds=([0.0, 0.0, 0.0, 0.0], [np.inf, 1.0, 1.0, 1.0]), fit_method="trr"
    )
...
def test_with_higher_S0():
    ...
    # This gives an isotropic signal.
    signal2 = multi_tensor(
        gtab, mevals2, snr=None, S0=S0_2, fractions=[f * 100, 100 * (1 - f)]
    )
    # Single voxel data
...
def test_fit_object():
    ...
    assert_raises(TypeError, ivim_fit_multi.__getitem__, -0.1, 0)
...
def test_fit_one_stage():
    ...
    linear_fit_params = [9.88834140e02, 1.19707191e-01, 7.91176970e-03, 9.30095210e-04]
...
def test_leastsq_failing():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        message = [
            "x0 obtained from linear fitting is not feasible",
            "x0 is unfeasible",
            "Bounds are violated for leastsq fitting",
        ]
        assert_greater_equal(
            len([lw for lw in u_warn for m in message if m in str(lw.message)]), 3
        )
