...
def generate_signal_crossing(gtab, lambda1, lambda2, lambda3, angle2=60):
    ...
    angl = [(0, 0), (angle2, 0)]
    S, _ = multi_tensor(gtab, mevals, S0=1.0, angles=angl, fractions=[50, 50], snr=None)
    return S
...
def test_input_parameters():
    ...
    assert_raises(
        ValueError, qtdmri.QtdmriModel, gtab_4d, laplacian_regularization="test"
    )
    ...
    assert_raises(
        ...
        gtab_4d,
        laplacian_regularization=True,
        laplacian_weighting=-1.0,
    )

    # non-bool for l1_weighting
    assert_raises(ValueError, qtdmri.QtdmriModel, gtab_4d, l1_regularization="test")
    ...
    assert_raises(ValueError, qtdmri.QtdmriModel, gtab_4d, cartesian="test")

    # non-bool anisotropic_scaling is caught
    assert_raises(ValueError, qtdmri.QtdmriModel, gtab_4d, anisotropic_scaling="test")
    ...
    assert_raises(ValueError, qtdmri.QtdmriModel, gtab_4d, eigenvalue_threshold=-1)
...
def test_orthogonality_temporal_basis_functions():
    ...
    int1 = integrate.quad(
        lambda t: qtdmri.temporal_basis(1, ut, t) * qtdmri.temporal_basis(2, ut, t),
        tmin,
        tmax,
    )
    int2 = integrate.quad(
        lambda t: qtdmri.temporal_basis(2, ut, t) * qtdmri.temporal_basis(3, ut, t),
        tmin,
        tmax,
    )
    int3 = integrate.quad(
        lambda t: qtdmri.temporal_basis(3, ut, t) * qtdmri.temporal_basis(4, ut, t),
        tmin,
        tmax,
    )
    int4 = integrate.quad(
        lambda t: qtdmri.temporal_basis(4, ut, t) * qtdmri.temporal_basis(5, ut, t),
        tmin,
        tmax,
    )
...
def test_anisotropic_isotropic_equivalence(radial_order=4, time_order=2):
    ...
    # regularization
    qtdmri_mod_aniso = qtdmri.QtdmriModel(
        gtab_4d,
    ...
    qtdmri_mod_iso = qtdmri.QtdmriModel(
        gtab_4d,
        radial_order=radial_order,
        time_order=time_order,
        cartesian=False,
...
def test_cartesian_normalization(radial_order=4, time_order=2):
    ...
    qtdmri_mod_aniso_norm = qtdmri.QtdmriModel(
        gtab_4d,
        radial_order=radial_order,
        time_order=time_order,
        cartesian=True,
...
def test_q0_constraint_and_unity_of_ODFs(radial_order=6, time_order=2):
    ...
    E_q0_first_tau = fitted_signal[np.all([tau == tau.min(), gtab_4d.b0s_mask], axis=0)]
    ...
    E_q0_first_tau = fitted_signal[np.all([tau == tau.min(), gtab_4d.b0s_mask], axis=0)]
    E_q0_last_tau = fitted_signal[np.all([tau == tau.max(), gtab_4d.b0s_mask], axis=0)]
    assert_almost_equal(E_q0_first_tau[0], 1.0)
    assert_almost_equal(E_q0_last_tau[0], 1.0)
...
@needs_cvxpy
def test_spherical_laplacian_reduces_laplacian_norm(radial_order=4, time_order=2):
    gtab_4d = generate_gtab4D()
    l1, l2, l3 = [0.0015, 0.0003, 0.0003]
    S = generate_signal_crossing(gtab_4d, l1, l2, l3)
...
def test_laplacian_GCV_higher_weight_with_noise(radial_order=4, time_order=2):
    ...
    l1, l2, l3 = [0.0015, 0.0003, 0.0003]
    S = generate_signal_crossing(gtab_4d, l1, l2, l3)
    S_noise = add_noise(S, S0=1.0, snr=10)
...
def test_visualise_gradient_table_G_Delta_rainbow():
    ...
    assert_raises(ValueError, qtdmri.visualise_gradient_table_G_Delta_rainbow, gtab_4d)
