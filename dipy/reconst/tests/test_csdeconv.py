...
def test_auto_response_ssst():
    ...
    response_auto, ratio_auto = auto_response_ssst(
        gtab, data, roi_center=None, roi_radii=(1, 1, 0), fa_thr=0.7
    )
...
def test_csdeconv():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert_greater(len([lw for lw in w if issubclass(lw.category, UserWarning)]), 0)
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert_equal(len([lw for lw in w if issubclass(lw.category, UserWarning)]), 0)
...
def test_r2_term_odf_sharp():
    ...
    with warnings.catch_warnings():
        ...
        sdt_model = ConstrainedSDTModel(gtab, ratio=3 / 15.0, sh_order=8)
...
def test_csd_predict():
    ...
    npt.assert_array_equal(prediction1[..., gtab.b0s_mask], 1.0)

    # Same with a different S0
    prediction1 = csd_fit.predict(S0=123.0)
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
            message=descoteaux07_legacy_msg,
        ...
        prediction2 = csd.predict(csd_fit.shm_coeff, S0=123.0)
    npt.assert_array_equal(prediction1, prediction2)
    npt.assert_array_equal(prediction1[..., gtab.b0s_mask], 123.0)

    # For "well behaved" coefficients, the model should be able to find the
    # coefficients from the predicted signal.
    coeff = np.random.random(csd_fit.shm_coeff.shape) - 0.5
    coeff[..., 0] = 10.0
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
...
def test_default_lambda_csdmodel():
    ...
    for sh_order, expected, e_warn in zip(
        expected_lambda.keys(),
        expected_lambda.values(),
        expected_csdmodel_warnings.values(),
    ):
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always", category=PendingDeprecationWarning)
            model_full = ConstrainedSphericalDeconvModel(
                gtab, response, sh_order=sh_order, reg_sphere=sphere
            )
...
def test_csd_superres():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        model16 = ConstrainedSphericalDeconvModel(gtab, (evals[0], 3.0), sh_order=16)
...
def test_csd_convergence():
    ...
    with warnings.catch_warnings():
        ...
        model_wo_conv = ConstrainedSphericalDeconvModel(
            gtab, (evals[0], 3.0), sh_order=8
        )
