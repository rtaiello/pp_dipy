...
def setup():
    ...
    data.mevals = np.array(([0.0015, 0.0003, 0.0003], [0.0015, 0.0003, 0.0003]))
    data.angl = [(0, 0), (60, 0)]
    data.S, sticks = multi_tensor(
...
def test_shore_error():
    ...
    npt.assert_raises(
        ValueError, ShoreModel, data.gtab, constrain_e0=False, positive_constraint=True
    )
...
def compute_e0(shorefit):
    ...
    for n in range(int(shorefit.model.radial_order / 2) + 1):
        signal_0 += shorefit.shore_coeff[n] * (
            genlaguerre(n, 0.5)(0)
            * (
                (factorial(n))
                / (2 * np.pi * (shorefit.model.zeta**1.5) * gamma(n + 1.5))
            )
            ** 0.5
        )
