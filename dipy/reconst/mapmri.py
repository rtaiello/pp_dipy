        ...
        if laplacian_regularization:
            ...
            if isinstance(laplacian_weighting, str):
                if not laplacian_weighting == "GCV":
                    raise ValueError(msg)
            elif isinstance(laplacian_weighting, float) or isinstance(
                laplacian_weighting, np.ndarray
            ):
...
def mapmri_isotropic_M_mu_dependent(radial_order, mu, qval):
    ...
    for n in range(0, radial_order + 1, 2):
        for j in range(1, 2 + n // 2):
            l = n + 2 - 2 * j
            const = (
                mu**l * np.exp(-pi2q2mu2) * genlaguerre(j - 1, l + 0.5)(2 * pi2q2mu2)
            )
            for m in range(-l, l + 1):
...
def mapmri_isotropic_K_mu_independent(radial_order, rgrad):
    ...
    for n in range(0, radial_order + 1, 2):
        for j in range(1, 2 + n // 2):
            l = n + 2 - 2 * j
            const = (
                (-1) ** (j - 1) * (np.sqrt(2) * np.pi) ** (-1) * (r**2 / 2) ** (l / 2)
            )
            for m in range(-l, l + 1):
                K[:, counter] = const * real_sh_descoteaux_from_index(m, l, theta, phi)
                counter += 1
...
def mapmri_isotropic_odf_matrix(radial_order, mu, s, vertices):
    ...
    for n in range(0, radial_order + 1, 2):
        for j in range(1, 2 + n // 2):
            ...
            for m in range(-l, l + 1):
                odf_mat[:, counter] = (
                    kappa * matsum * real_sh_descoteaux_from_index(m, l, theta, phi)
                )
                counter += 1
