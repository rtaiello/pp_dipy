...
def iter_fit_tensor(step=1e4):
    ...
    def iter_decorator(fit_tensor):
        ...
        def wrapped_fit_tensor(
            ...
            if step >= size:
                return fit_tensor(
                    design_matrix, data, return_S0_hat=return_S0_hat, *args, **kwargs
                )
            ...
            for i in range(0, size, step):
                if return_S0_hat:
                    dtiparams[i : i + step], S0params[i : i + step] = fit_tensor(
                        design_matrix,
                        data[i : i + step],
                    ...
                    dtiparams[i : i + step] = fit_tensor(
                        design_matrix, data[i : i + step], *args, **kwargs
                    )
            if return_S0_hat:
                return (
                    dtiparams.reshape(shape + (12,)),
...
def wls_fit_tensor(design_matrix, data, return_S0_hat=False):
    ...
    fit_result = np.einsum("...ij,...j", pinv(design_matrix * w[..., None]), w * log_s)
    if return_S0_hat:
        return (
...
def _nlls_err_func(tensor, design_matrix, data, weighting=None, sigma=None):
    r"""
...
def nlls_fit_tensor(
    design_matrix, data, weighting=None, sigma=None, jac=True, return_S0_hat=False
):
    """
    ...
    for vox in range(flat_data.shape[0]):
        ...
        if jac:
            this_param, status = opt.leastsq(
                _nlls_err_func,
                start_params,
                args=(design_matrix, flat_data[vox], weighting, sigma),
        ...
        # OLS solution in this voxel:
            ...
            evals, evecs = decompose_tensor(from_lower_triangular(this_params[:6]))
            params[vox, :3] = evals
            params[vox, 3:12] = evecs.ravel()
    ...
    for vox in range(flat_data.shape[0]):
        ...
        if jac:
            this_param, status = opt.leastsq(
                _nlls_err_func,
                start_params,
                args=(design_matrix, flat_data[vox], "sigma", sigma),
                Dfun=_nlls_jacobian_func,
            )
        else:
            this_param, status = opt.leastsq(
            ...
            if np.any(np.abs(residuals) > 3 * sigma):
                ...
                if jac:
                    this_param, status = opt.leastsq(
                        _nlls_err_func,
                        start_params,
                        args=(clean_design, clean_sig, "sigma", this_sigma),
                        Dfun=_nlls_jacobian_func,
                    )
                else:
                    this_param, status = opt.leastsq(
