...
from dipy.core.geometry import sphere2cart, cart2sphere, perpendicular_directions
...
class DiffusionKurtosisModel(ReconstModel):
    ...
    def fit(self, data, mask=None):
        ...
        params_in_mask = self.fit_method(
            self.design_matrix, data_in_mask, *self.args, **self.kwargs
        )
...
def ols_fit_dki(design_matrix, data):
    ...
    for vox in range(len(data_flat)):
        dki_params[vox] = _ols_iter(inv_design, data_flat[vox], min_diffusivity)
...
def wls_fit_dki(design_matrix, data):
    ...
    for vox in range(len(data_flat)):
        dki_params[vox] = _wls_iter(
            design_matrix, inv_design, data_flat[vox], min_diffusivity
        )
