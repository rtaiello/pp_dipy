...
@deprecate_with_version(
    "dipy.io.bvectxt module is deprecated, "
    "Please use dipy.core.gradients module instead",
    since="1.4",
    until="1.5",
)
def read_bvec_file(filename, atol=0.001):
    """
    ...
    if b_values.shape[0] != grad_table.shape[1]:
        raise IOError(
            "the gradient file and b value file should"
    ...
    if not np.allclose(grad_norms[b_values > 0], 1, atol=atol):
        raise IOError(
            "the magnitudes of the gradient directions"
            + "are not within "
            + str(atol)
            + " of 1"
        )
...
def reorient_vectors(bvecs, current_ornt, new_ornt, axis=0):
    ...
    """
    return gradients.reorient_vectors(
        bvecs=bvecs, current_ornt=current_ornt, new_ornt=new_ornt, axis=axis
    )
...
def reorient_on_axis(bvecs, current_ornt, new_ornt, axis=0):
    return gradients.reorient_on_axis(
        bvecs=bvecs, current_ornt=current_ornt, new_ornt=new_ornt, axis=axis
    )
