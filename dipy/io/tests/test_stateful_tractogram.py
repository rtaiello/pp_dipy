...
def switch_voxel_sizes_from_rasmm():
    ...
    sft_switch = StatefulTractogram(
        sft.streamlines, filepath_dix["gs_3mm.nii"], Space.RASMM
    )
    tmp_points_rasmm = np.loadtxt(filepath_dix["gs_rasmm_space.txt"])
    tmp_points_voxmm = np.loadtxt(filepath_dix["gs_voxmm_space.txt"])
...
def switch_voxel_sizes_from_voxmm():
    ...
    sft_switch = StatefulTractogram(
        sft.streamlines, filepath_dix["gs_3mm.nii"], Space.VOXMM
    )
    tmp_points_rasmm = np.loadtxt(filepath_dix["gs_rasmm_space.txt"])
    tmp_points_voxmm = np.loadtxt(filepath_dix["gs_voxmm_space.txt"])
...
def reassign_both_data_sep_to_empty():
    ...
    return sft.data_per_point == {} and sft.data_per_streamline == {}
...
def test_replace_streamlines():
    ...
    assert_(subsample_streamlines(), msg="Subsampling streamlines should not fail")
    assert_(replace_streamlines(), msg="Replacing streamlines should not fail")
    assert_(
        reassign_both_data_sep(),
        msg="Reassigning streamline/point data should not fail",
    )
    assert_(
        reassign_both_data_sep_to_empty(),
...
def test_bounding_box():
    assert_(
        bounding_bbox_valid(Origin.NIFTI),
        msg="Bounding box should be valid with proper declaration",
    )
    assert_(
        bounding_bbox_valid(Origin.TRACKVIS),
        msg="Bounding box should be valid with proper declaration",
    )
    assert_(
        not out_of_grid(100), msg="Positive translation should make the bbox check fail"
    )
    assert_(
        not out_of_grid(-100),
...
def test_trk_coloring():
    assert_(random_streamline_color(), msg="Streamlines color assignment failed")
    assert_(random_point_gray(), msg="Streamlines points gray assignment failed")
    assert_(random_point_color(), msg="Streamlines points color assignment failed")
