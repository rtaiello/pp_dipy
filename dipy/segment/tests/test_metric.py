...
def norm(x, order=None, axis=None):
    if axis is not None:
        return np.apply_along_axis(np.linalg.norm, axis, x.astype(np.float64), order)
...
def test_metric_minimum_average_direct_flip():
    ...
    for metric in [
        ...
        assert_equal(metric.dist(s, s), 0.0)
        assert_equal(metric.dist(s, s[::-1]), 0.0)
        ...
        M_rotation = rodrigues_axis_rotation(rot_axis, 60.0).astype(dtype)
        s_mean = np.mean(s, axis=0)
        s_zero_mean = s - s_mean
...
def test_metric_cosine():
    ...
    class CosineMetric(dipysmetric.Metric):
        ...
        def dist(self, v1, v2):
            ...
            if norm(v1) == 0:
                return 0.0 if norm(v2) == 0 else 1.0
    ...
    for metric in [CosineMetric(feature), dipysmetric.CosineMetric(feature)]:
        ...
        v2 = np.array([[1, -1.0 / 2, 0]], dtype=np.float32)
        v3 = np.array([[-1, -2, -3]], dtype=np.float32)

        assert_equal(metric.dist(v0, v0), 0.0)  # dot-dot
        assert_equal(metric.dist(v0, v1), 1.0)  # dot-line
        assert_equal(metric.dist(v1, v1), 0.0)  # collinear
        assert_equal(metric.dist(v1, v2), 0.5)  # orthogonal
        assert_equal(metric.dist(v1, v3), 1.0)  # opposite
    ...
    for dtype in [np.int32, np.int64, np.float32, np.float64]:
        ...
        for i in range(len(data)):
            for j in range(len(data)):
                assert_equal(D[i, j], dipysmetric.dist(metric, data[i], data[j]))
        ...
        for i in range(len(data)):
            for j in range(len(data2)):
                assert_equal(D[i, j], dipysmetric.dist(metric, data[i], data2[j]))
