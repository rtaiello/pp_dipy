...
class RecoBundles(object):
    def __init__(
        ...
        if self.verbose:
            ...
            logger.info(
                "After refining target brain streamlines"
        ...
        if cluster_map is None:
            ...
            if self.verbose:
                logger.info(" Streamlines have %d centroids" % (self.nb_centroids,))
    ...
    def _cluster_streamlines(self, clust_thr, nb_pts):
        if self.verbose:
            ...
            logger.info(" Tractogram has %d streamlines" % (len(self.streamlines),))
    ...
    @deprecated_params("slr_num_threads", "num_threads", since="1.4", until="1.5")
    def recognize(
        self,
        model_bundle,
        model_clust_thr,
        ...
        if self.verbose:
            logger.info(
                "Total duration of recognition time is %0.3f sec.\n" % (time() - t,)
            )
    ...
    def _cluster_model_bundle(
        ...
        if self.verbose:
            ...
            logger.info(" Model bundle has %d streamlines" % (len(model_bundle),))
        ...
        if self.verbose:
            logger.info(" Model bundle has %d centroids" % (nb_model_centroids,))
    ...
    def _reduce_search_space(
        ...
        if nb_neighb_streamlines == 0:
            ...
            return Streamlines([]), []
        if self.verbose:
            logger.info(" Number of neighbor streamlines %d" % (nb_neighb_streamlines,))
            logger.info(" Duration %0.3f sec. \n" % (time() - t,))
