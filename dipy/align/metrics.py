...
from dipy.mpc import mpc_crosscorr as mpc_cc
from dipy.mpc import mpc_sumsqdiff as mpc_ssd
...
class SimilarityMetric(object, metaclass=abc.ABCMeta):
    ...
    def compute_forward(self):
        ...
        Computes the forward update field to model the moving image towards
    ...
    def compute_backward(self):
        ...
        Computes the backward update field to model the static image towards
...
class CCMetric(SimilarityMetric):
    def __init__(self, dim, sigma_diff=2.0, radius=4, mpc=False):
    ...
    def _connect_functions(self):
        ...
        if self.dim == 2:
            ...
            if self.mpc:
                self.precompute_factors = mpc_cc.precompute_cc_factors_3d_fast
                self.compute_forward_backward_step = mpc_cc.compute_cc_forward_backward_step_3d
    ...
    def initialize_iteration(self):
        ...
        if invalid_image_size(self.static_image):
            raise ValueError(
                "Static image size is too small. "
                + msg % dict(shape=self.static_image.shape)
            )
        if invalid_image_size(self.moving_image):
            raise ValueError(
                "Moving image size is too small. "
                + msg % dict(shape=self.moving_image.shape)
            )
        if self.mpc:
            self.factors, self.share_factor_2, num_bits = self.precompute_factors(
                self.static_image, self.moving_image, self.radius
            )
            self.num_bits += num_bits
            def reconstruct(shares):
                return sum(shares) % 6497992661811505123

            def decode(field_element,precision_fractional=5):
                Q = 6497992661811505123
                BASE = 10
                upscaled = field_element if field_element <= Q/2 else field_element - Q
                rational = upscaled / BASE**precision_fractional
                return rational
            # for s in range(self.share_factor_2.shape[0]):
            #     for c in range(self.share_factor_2.shape[1]):
            #         for r in range(self.share_factor_2.shape[2]):
            #             fst = int(self.share_factor_2[s,c,r][0])
            #             snd  = int(self.share_factor_2[s,c,r][1])
            #             print("reconstruct")
            #             print(decode(reconstruct([fst,snd])))
            #             print("factors")
            #             print(self.factors[s,c,r,2])
        else:
    ...
    def compute_forward(self):
        ...
        """
        if self.mpc:
            displacement_forward, displacement_backward, self.energy, num_bits = self.compute_forward_backward_step(
                self.gradient_static, self.gradient_moving, self.factors, self.share_factor_2, self.radius
            )
            self.num_bits += num_bits
            displacement = displacement_forward
            self.displacement_backward = displacement_backward
        else:
    ...
    def compute_backward(self):
        ...
        """
        if self.mpc:
            displacement = self.displacement_backward
        else:
    ...
    def get_num_bits(self):
        r"""Number of bits used by this metric to compute the current image pair

        Returns the number of bits used by this metric to compute the current
        image pair
        """
...
class SSDMetric(SimilarityMetric):
    def __init__(self, dim, smooth=4, inner_iter=10, step_type="demons", mpc=False):
    ...
    def compute_demons_step(self, forward_step=True):
        ...
        if self.dim == 2:
            if self.mpc:
                delta_sq_mean = np.mean(delta_field**2)
                self.energy = np.sum(delta_field**2)
                if forward_step:
                    gradient = self.gradient_static
                    step_static = mpc_ssd.compute_ssd_demons_step_2d(
                    self.static_image, gradient, sigma_reg_2,delta_sq_mean, None)
                    step_moving = mpc_ssd.compute_ssd_demons_step_2d(
                    self.moving_image, gradient, sigma_reg_2,delta_sq_mean, None)
                    step = step_static - step_moving 
            ...
            if self.mpc:
                ...
                if forward_step:
                    gradient = self.gradient_static
                    step_static = mpc_ssd.compute_ssd_demons_step_3d(
                    self.static_image, gradient, sigma_reg_2,delta_sq_mean, None)
                    step_moving = mpc_ssd.compute_ssd_demons_step_3d(
                    self.moving_image, gradient, sigma_reg_2,delta_sq_mean, None)
                    step = step_static - step_moving
                else:
                    gradient = self.gradient_moving
...
def v_cycle_2d(
    ...
    subgradient_field = np.array(vfu.downsample_displacement_field_2d(gradient_field))
    ...
    displacement += vfu.resample_displacement_field_2d(
        sub_displacement, np.array([0.5, 0.5]), shape
    )
...
def v_cycle_3d(
    ...
    subgradient_field = np.array(vfu.downsample_displacement_field_3d(gradient_field))
    shape = np.array(displacement.shape).astype(np.int32)
    ...
    del sub_residual
    displacement += vfu.resample_displacement_field_3d(
        sub_displacement, 0.5 * np.ones(3), shape
    )
