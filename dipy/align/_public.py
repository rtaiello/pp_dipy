    ...
    **metric_kwargs,
    ...
    for func in pipeline:
        if func == "center_of_mass":
            ...
            current_iter += affreg.iter
    # After doing all that, resample once at the end:
    affine_map = AffineMap(
        starting_affine, static.shape, static_affine, moving.shape, moving_affine
    )
    ...
    return resampled, starting_affine, use_metric,  current_iter
...
def register_series(
    ...
    ref : int or 3D array or nib.Nifti1Image class instance or str
        ...
        series. Otherwise it is an array of data to model with (associated
    ...
    data that was used to transform it into model with the static data.
...
def register_dwi_series(
    ...
    if np.sum(gtab.b0s_mask) > 1:
        # First, model the b0s into one image and average:
        ...
        # There's only one b0 and we model everything to it
