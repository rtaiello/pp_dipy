...
def test_random_displacement_field_2d():
    ...
    assert_raises(
        ValueError, vfu.create_random_displacement_2d, shape, invalid, shape, valid
    )
    assert_raises(
        ValueError, vfu.create_random_displacement_2d, shape, valid, shape, invalid
    )
...
def test_reorient_random_vector_fields():
    ...
    for n_dims, func in (
        ...
        affine = from_matvec(np.random.normal(size=(n_dims, n_dims)), np.zeros(n_dims))
        func(arr_32, affine)
