...
def iterate_residual_field_ssd_2d(
    delta_field, sigmasq_field, grad, target, lambda_param, dfield
):
    r"""
...
def iterate_residual_field_ssd_3d(
    delta_field, sigmasq_field, grad, target, lambda_param, dfield
):
    r"""
    This implementation is for testing purposes only. The problem
...
def test_compute_residual_displacement_field_ssd_2d():
    ...
    for it in range(2):
        ...
        expected[inf_sigma == 0, 0] -= (
            grad_G[inf_sigma == 0, 0] * dp[inf_sigma == 0]
            + sigma_field[inf_sigma == 0] * s[inf_sigma == 0, 0]
        )
        expected[inf_sigma == 0, 1] -= (
            grad_G[inf_sigma == 0, 1] * dp[inf_sigma == 0]
...
def test_compute_residual_displacement_field_ssd_3d():
    ...
    for it in range(2):
        ...
        # Expected residuals when sigma != infinity
        for i in range(3):
            expected[inf_sigma == 0, i] -= (
                grad_G[inf_sigma == 0, i] * dp[inf_sigma == 0]
