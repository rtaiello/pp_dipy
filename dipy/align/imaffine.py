...
from dipy.align.parzenhist import (ParzenJointHistogram,
                                   ...
                                   compute_parzen_mi)
from dipy.mpc.mpc_parzenhist import ParzenJointHistogramMPC
...
class MutualInformationMetric:
    ...
    def __init__(self, nbins=32, sampling_proportion=None, mpc=False):
        ...
        print("called")
        if mpc:
            self.histogram = ParzenJointHistogramMPC(nbins)
        else:
...
class AffineRegistration:
    ...
    def optimize(self, static, moving, transform, params0,
        ...
        for level in range(self.levels - 1, 0, -1): #for level in range(self.   levels - 1, -1, -1):
            ...
            params = opt.xopt
            self.iter += opt.res.nit
            print("opt.res.nit:", opt.res.nit)
