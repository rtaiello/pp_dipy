...
def test_voxel2streamline():
    ...
    npt.assert_equal(
        v2fn, {0: {0: [0], 1: [1], 2: [2], 3: [3]}, 1: {0: [0], 1: [1], 2: [2]}}
    )
    affine = np.array(
        [[0.9, 0, 0, 10], [0, 0.9, 0, -100], [0, 0, 0.9, 2], [0, 0, 0, 1]]
    )
...
def test_FiberModel_init():
    ...
    for sphere in [None, False, dpd.get_sphere("symmetric362")]:
        ...
        npt.assert_equal(
            fiber_matrix.shape, (len(vox_coords) * 64, len(streamline_cases[0]))
        )
        npt.assert_raises(
            IndexError, FM.setup, streamline_cases[1], affine, sphere=sphere
        )
...
def test_FiberFit():
    ...
    this_data[vox_coords[:, 0], vox_coords[:, 1], vox_coords[:, 2]] = (
        sig.reshape((4, 64)) * S0[vox_coords[:, 0], vox_coords[:, 1], vox_coords[:, 2]]
    )
