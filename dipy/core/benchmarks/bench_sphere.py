...
def bench_disperse_charges_alt():
    ...
    fig = plt.figure(
        num="Electrostatic repulsion methods benchmark", figsize=figsize, dpi=dpi
    )
    for idx, subplot_index in enumerate(["131", "132", "133"]):
        ...
        if mode == "subdivide":
            ...
            init_pointset = sphere_stats.random_uniform_on_sphere(num_points[idx])
            ...
            for num_iterations in range(12):
                ...
                with timer:
                    for i in range(num_repetitions):
                        opt = func_minimize_adhoc(init_hemisphere, num_iterations_dipy)
                execution_time_adhoc.append(
                    timer.duration_in_seconds() / num_repetitions
                )
                ...
                execution_time_scipy.append(
                    timer.duration_in_seconds() / num_repetitions
                )
                minimum_scipy.append(sphere._get_forces_alt(opt.ravel()))

        ax = fig.add_subplot(subplot_index)
        ax.plot(execution_time_adhoc, minimum_adhoc, "r+", label="DIPY original")
        ax.plot(execution_time_scipy, minimum_scipy, "g+", label="SciPy-based")
        ax.set_yscale("log")
