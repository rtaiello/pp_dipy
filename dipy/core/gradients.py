...
class GradientTable(object):
    ...
    def __init__(
        ...
        if btens is not None:
            ...
            elif isinstance(btens, np.ndarray) and (
                btens.shape == (gradients.shape[0],)
                or (btens.shape == (gradients.shape[0], 1))
                or (btens.shape == (1, gradients.shape[0]))
            ):
                b_tensors = np.zeros((len(self.bvals), 3, 3))
...
def orientation_from_string(string_ornt):
    ...
    orientation_dict = dict(
        r=(0, 1), l=(0, -1), a=(1, 1), p=(1, -1), s=(2, 1), i=(2, -1)
    )
