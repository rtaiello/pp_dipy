...
def cyproc_exts(
    exts, cython_min_version, hash_stamps_fname="pyx-stamps", build_ext=build_ext
):
    ...
    try:
        ...
        return (
            derror_maker(
                ...
                'but cannot import "Cython"'.format(cython_min_version),
            ),
            True,
        )
    ...
    return (
        derror_maker(
            build_ext,
            "Need cython>={0} to build extensions"
            "but found cython version {1}".format(cython_min_version, cyversion),
        ),
        True,
    )
...
def build_stamp(pyxes, include_dirs=()):
    ...
    for source in pyxes:
        ...
        pyx_defs[source] = dict(pyx_hash=pyx_hash, c_filename=c_filename, c_hash=c_hash)
    return pyx_defs
...
def get_pyx_sdist(sdist_like=sdist, hash_stamps_fname="pyx-stamps", include_dirs=()):
